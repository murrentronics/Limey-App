-- Create complete likes system from scratch
-- This creates the table, functions, triggers, and permissions
...
-- 2. Add like_count column to videos table if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'videos' AND column_name = 'like_count' AND table_schema = 'public') THEN
        ALTER TABLE videos ADD COLUMN like_count INTEGER DEFAULT 0 NOT NULL;
    END IF;
END $$;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'user_has_liked_video' AND routine_schema = 'public') THEN
        RAISE NOTICE 'Success: user_has_liked_video function created';
    ELSE
        RAISE NOTICE 'Error: user_has_liked_video function not found';
    END IF;
END $$;